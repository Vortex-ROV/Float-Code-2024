...
#define POT_PIN 34
#define POT_LOWER_LIMIT (100 + 50)
#define POT_UPPER_LIMIT (2800 - 50)
// #define DIR_UP LOW
...
#define DISTANCE_LOWER_LIMIT 55  // Example limit in mm
#define DISTANCE_UPPER_LIMIT 85 // Example limit in mm
...
int readDistance() {
  ...
  laser = sensor.readRangeSingleMillimeters() * 0.01f + laser * 0.99f;
...
void setup() {
  ...
  // go up
  while (readDistance() <= DISTANCE_UPPER_LIMIT) {
    moveMotorUp();
  }
...
void loop() {
  ...
  if (!done && currentMillis - previousMillis <= interval) {
    ...
    if (readDistance() > requiredDistance + 2) {
      // go down
      moveMotorDown();
    } else if (readDistance() < requiredDistance - 2) {
      // go up
      moveMotorUp();
    } else {
      // stop motor
